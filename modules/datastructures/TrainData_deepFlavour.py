...
class TrainData_DF_puppi(TrainData):
    def __init__(self):

        TrainData.__init__(self)

        
        self.description = "DeepJet puppi training datastructure"
        
        self.truth_branches = ['isB','isBB','isGBB','isLeptonicB','isLeptonicB_C','isC','isGCC','isCC','isUD','isS','isG','isPU']
        self.undefTruth=['isUndefined']
        self.weightbranchX='jet_pt'
        self.weightbranchY='jet_eta'
        self.remove = True
        ...
        self.weight_binY = np.array(
            [-5.0,-3.5,-2.5,-2.,-1.5,-1.,-0.5,0.5,1,1.5,2.,2.5,3.5,5.0],
            dtype=float
        )
        ...
        self.reg_truth = ['gen_pt_WithNu']


        self.reduced_truth = ['isB','isBB','isLeptonicB','isC','isUDS','isG','isPU']
    ...
    def convertFromSourceFile(self, filename, weighterobjects, istraining):
        ...
        def reduceTruth(uproot_arrays):
            ...
            pu = uproot_arrays[b'isPU']
                
    
            return np.vstack((b,bb+gbb,lepb,c+cc+gcc,uds,g,pu)).transpose()
        ...
        x_npf = np.where(np.isfinite(x_npf), x_npf, 0)
        x_vtx = np.where(np.isfinite(x_vtx), x_vtx, 0)
        reg_truth = np.where(np.isfinite(reg_truth), reg_truth, 1)

        return [x_global,x_cpf,x_npf,x_vtx], [truth,reg_truth,reg_truth,reg_truth], []
    ...
    def writeOutPrediction(self, predicted, features, truth, weights, outfilename, inputfile):
        ...
        from root_numpy import array2root
        out = np.core.records.fromarrays(np.vstack( (predicted[0].transpose(),truth[0].transpose(), features[0][:,0:2].transpose(),
                                                     predicted[1].transpose(), predicted[2].transpose(),predicted[3].transpose(), truth[1].transpose() ) ),
                                         names='prob_isB, prob_isBB,prob_isLeptB, prob_isC,prob_isUDS,prob_isG,prob_isPU,isB, isBB, isLeptB, isC,isUDS,isG,isPU,jet_pt, jet_eta,pred_corr_factor,pred_down_corr_factor,pred_up_corr_factor,truth_corr_factor')
