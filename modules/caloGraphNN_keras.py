...
class GravNet(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.output_dim = self.compute_output_shape(input_shape)
...
class GravNet_vertex(keras.layers.Layer):
    def __init__(self, n_neighbours, n_dimensions, n_filters, n_propagate, **kwargs):
        super(GravNet_vertex, self).__init__(**kwargs)
        ...
        self.input_feature_transform = keras.layers.Dense(n_propagate)
        self.input_spatial_transform = keras.layers.Dense(n_dimensions)
        self.output_feature_transform = keras.layers.Dense(n_filters, activation='relu')
    ...
    def build(self, input_shape):
        self.input_feature_transform.build(input_shape)
        self.input_spatial_transform.build(input_shape)
        self.output_feature_transform.build((input_shape[0], input_shape[1], input_shape[2], input_shape[3] + self.input_feature_transform.units * 2))
        self.output_dim = self.compute_output_shape(input_shape)
        for layer in self._sublayers:
            self._trainable_weights.extend(layer.trainable_weights)
            self._non_trainable_weights.extend(layer.non_trainable_weights)
        
        super(GravNet_vertex, self).build(input_shape)

    def call(self, x):
        features = self.input_feature_transform(x)
        coordinates = self.input_spatial_transform(x)

        collected_neighbours = self.collect_neighbours_vertex(coordinates, features)
    ...
    def compute_output_shape(self, input_shape):
        return (input_shape[0], input_shape[1], input_shape[2], self.output_feature_transform.units)
    ...
    def collect_neighbours_vertex(self, coordinates, features):
        # V = number of vertices
        # N = number of neighbours
        # F = number of features per vertex
    ...
    def get_config(self):
        config = {'n_neighbours': self.n_neighbours, 'n_dimensions': self.n_dimensions, 'n_filters': self.n_filters, 'n_propagate': self.n_propagate}
        base_config = super(GravNet_vertex, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
