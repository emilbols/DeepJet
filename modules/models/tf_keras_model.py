...
def _particle_net_base(flat, neutrals, features=None, mask=None, vertex=None, reg=None, setting=None, name='particle_net'):
    # points : (N, P, C_coord)
    ...
    with tf.name_scope(name):
        ...
        if setting.fc_params is not None:
            ...
            reg = keras.layers.concatenate([reg,out])
            reg_out = keras.layers.Dense(setting.num_targets, activation='linear')(reg)
            return [out,reg_out]  # (N, num_classes)
        else:
            return pool
...
def get_particle_net_model(Inputs,nclasses,nregclasses,dropoutRate=0.1,momentum=0.6):
    ...
    """
    setting = _DotDict()
    setting.num_class = nclasses
    setting.num_targets = nregclasses
    ...
    setting.fc_params = [(256, dropoutRate)]
    ...
    vtx = Inputs[3]
    reg = Inputs[4]
    setting.num_points = 25

    mask = None

    outputs = _particle_net_base(flat,npf, cpf, mask, vtx, reg, setting, name='ParticleNet')

    return keras.Model(inputs=[flat, cpf, npf, vtx, reg], outputs=outputs, name='ParticleNet')
